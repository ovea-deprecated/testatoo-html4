...
import static org.testatoo.cartridge.html4.element.HtmlElementType.Dt;

/**
 * This class allows the testing of the "dt" element properties for html4.
 * Dt is a definition term, used in conjunction with dl to define a definition list and dd, to define the description linked with the term.
 *
 * @author dev@testatoo.org
 * @see <a href="http://www.w3.org/TR/html401/struct/lists.html#edef-DT"> "dt" specification </a>
 */

public final class Dt extends Component implements Coreattrs, I18n {
     ...
     * @param id        the id (unique) of the "dt" element
     * @throws ComponentException if the given id does not correspond to a "dt" element
     */
    public Dt(HtmlEvaluator evaluator, String id) {
        super(evaluator, id);
        this.evaluator = evaluator;

        coreAttributeSupport = new CoreAttributeSupport(evaluator);
        i18nAttributeSupport = new I18nAttributeSupport(evaluator);
        if (evaluator.htmlElementType(id) != Dt) {
            throw new ComponentException("The component with id=" + id + " is not a " + Dt + " but a " + evaluator.htmlElementType(id));
        }
    }
     ...
     *
     * @return string describing the "dt" element
     */
