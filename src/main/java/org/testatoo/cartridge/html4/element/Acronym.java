...
import static org.testatoo.cartridge.html4.element.HtmlElementType.Acronym;

/**
 * This class allows the testing of the "acronym" properties of a structured text for html4.
 * Acronym Defines an element in a structured text that is made up of the first letters of the words of a phrase .
 *
 * @author dev@testatoo.org
 * @see <a href="http://www.w3.org/TR/html401/struct/text.html#edef-ACRONYM"> acronym specification </a>
 */

public final class Acronym extends Phrase implements Coreattrs, I18n {
     ...
     * @throws ComponentException if the given id does not correspond to a "acronym" element
     */
    public Acronym(HtmlEvaluator evaluator, String id) {
        super(evaluator, id);

        coreAttributeSupport = new CoreAttributeSupport(evaluator);
        i18nAttributeSupport = new I18nAttributeSupport(evaluator);
        if (evaluator.htmlElementType(id) != Acronym) {
            throw new ComponentException("The component with id=" + id + " is not a " + Acronym + " but a " + evaluator.htmlElementType(id));
        }
    }
