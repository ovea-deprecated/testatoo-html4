...
import static org.testatoo.cartridge.html4.element.HtmlElementType.Em;

/**
 * This class allows the testing of the "em" properties of a structured text for html4.
 * Em indicates emphasis of a text.
 *
 * @author dev@testatoo.org
 * @see <a href="http://www.w3.org/TR/html401/struct/text.html#edef-EM"> em specification </a>
 */

public final class Em extends Phrase implements Coreattrs, I18n {
     ...
     * @param id        the id (unique) of the em element
     * @throws ComponentException if the given id does not correspond to a "em" element
     */
    public Em(HtmlEvaluator evaluator, String id) {
        super(evaluator, id);

        coreAttributeSupport = new CoreAttributeSupport(evaluator);
        i18nAttributeSupport = new I18nAttributeSupport(evaluator);
        if (evaluator.htmlElementType(id) != Em) {
            throw new ComponentException("The component with id=" + id + " is not a " + Em + " but a " + evaluator.htmlElementType(id));
        }
    }
