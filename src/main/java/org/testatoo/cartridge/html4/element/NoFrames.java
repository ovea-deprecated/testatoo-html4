...
import static org.testatoo.cartridge.html4.element.HtmlElementType.NoFrames;

/**
 * This class allows the testing of the "noframes" element properties for html4.
 * Noframes is the alternate content container for non frame-based rendering
 *
 * @author dev@testatoo.org
 * @see <a href="http://www.w3.org/TR/html401/present/frames.html#edef-NOFRAMES"> noframes specification </a>
 */


public final class NoFrames extends Component implements Coreattrs, I18n {
     ...
     * @param id        the id (unique) of the "noframes" element
     * @throws ComponentException if the given id does not correspond to a "noframes" element
     */
    public NoFrames(HtmlEvaluator evaluator, String id) {
        super(evaluator, id);
        this.evaluator = evaluator;

        coreAttributeSupport = new CoreAttributeSupport(evaluator);
        i18nAttributeSupport = new I18nAttributeSupport(evaluator);
        if (evaluator.htmlElementType(id) != NoFrames) {
            throw new ComponentException("The component with id=" + id + " is not a " + NoFrames + " but a " + evaluator.htmlElementType(id));
        }
    }
     ...
     *
     * @return string describing the "noframes" element
     */
